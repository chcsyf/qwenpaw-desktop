...
- 📋 **粘贴板（展开式面板，UTF-8 中文无乱码）**：点击「📋」展开粘贴板列表——
  - 远程 Ctrl+C 复制 → 自动加入粘贴板（不再直接覆盖本地剪贴板），点击条目才复制到本地
  - 本地复制 → 自动读入粘贴板；点击条目或桌面内 Ctrl+V 自动填入远程
  - 输入框回车加入条目（Shift+Enter 换行）、重复文本自动去重置顶、上限 10 条
...
- 🔘 **底部状态栏工具按钮**：快捷入口 / 分辨率 / 截图 / 粘贴板 / 重连 / 关闭 6 个图标按钮（悬停有名称提示），连接状态实时显示；桌面视口内不再显示工具栏（独立访问 `desktop_page` 时工具栏仍保留）
